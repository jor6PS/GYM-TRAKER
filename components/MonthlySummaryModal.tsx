...
import { X, ShieldAlert, AlertTriangle, Target, FileText, Dumbbell, Save, Check, Activity, RefreshCw } from 'lucide-react';
import { Workout, GlobalReportData, User, WorkoutPlan, Exercise, TrainingGoal } from '../types';
...
const REPORT_CACHE_PREFIX = 'gymtraker:monthlyReport';
...
const buildReportSignature = (workouts: Workout[], user: User, catalogCount: number, trainingGoal: TrainingGoal | null): string => {
  let latestTs = 0;
  let rollingHash = 0;

  for (const w of workouts) {
    const ts = new Date(w.created_at || w.date).getTime();
    if (!Number.isNaN(ts) && ts > latestTs) latestTs = ts;
    rollingHash = (rollingHash + hashString(`${w.id}|${w.date}|${w.created_at || ''}|${w.user_weight ?? ''}`)) >>> 0;
  }
...
const getReportCacheKey = (userId: string, date = new Date()) => {
  const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
  return `${REPORT_CACHE_PREFIX}:${userId}:${monthKey}`;
};

const getGoalCacheKey = (userId: string, date = new Date()) => {
  const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
  return `${REPORT_CACHE_PREFIX}:goal:${userId}:${monthKey}`;
};

const TRAINING_GOALS: { id: TrainingGoal; label: string; desc: string }[] = [
  { id: 'fuerza', label: 'Fuerza', desc: 'Prioriza aumento de cargas y progresion.' },
  { id: 'hipertrofia', label: 'Hipertrofia', desc: 'Maximiza volumen y tension muscular.' },
  { id: 'mantenimiento', label: 'Mantenimiento', desc: 'Sostiene nivel con fatiga controlada.' },
  { id: 'adelgazar', label: 'Adelgazar', desc: 'Enfoca gasto calorico y adherencia.' },
  { id: 'resistencia', label: 'Resistencia', desc: 'Mejora capacidad de trabajo y densidad.' }
];

const getGoalLabel = (goal: TrainingGoal | null) => {
  if (!goal) return '';
  const match = TRAINING_GOALS.find((item) => item.id === goal);
  return match?.label || goal;
};
...
export const MonthlySummaryModal: React.FC<MonthlySummaryModalProps> = ({ isOpen, onClose, workouts, currentUser, onSavePlan }) => {
  ...
  const [forceRecalcTick, setForceRecalcTick] = useState(0);
  const { catalog } = useExercises();
  const isLoadingRef = useRef(false);
  const mountedRef = useRef(true);
  const forceRecalcRef = useRef(0);
  ...
  useEffect(() => {
    ...
    const signature = buildReportSignature(workouts || [], currentUser, catalogCount, trainingGoal);
    const cacheKey = getReportCacheKey(currentUser.id);
    const bypassCache = forceRecalcTick !== forceRecalcRef.current;
    if (bypassCache) {
      forceRecalcRef.current = forceRecalcTick;
    }

    if (typeof window !== 'undefined') {
      try {
        const cachedRaw = window.localStorage.getItem(cacheKey);
        if (cachedRaw && !bypassCache) {
          const cached = JSON.parse(cachedRaw) as { signature: string; data: GlobalReportData };
        ...
        // Ignorar errores de caché y continuar con el flujo normal
      }
    }
            ...
            if (mountedRef.current) {
              ...
              if (typeof window !== 'undefined') {
                try {
                  ...
                  // Si falla el almacenamiento, simplemente no cacheamos
                }
              }
            }
          })
  ...
  useEffect(() => {
    if (!isOpen || trainingGoal) return;
    try {
      if (typeof window === 'undefined') return;
      const cachedGoal = window.localStorage.getItem(getGoalCacheKey(currentUser.id));
      if (cachedGoal && TRAINING_GOALS.some((g) => g.id === cachedGoal)) {
        setTrainingGoal(cachedGoal as TrainingGoal);
      } else {
  ...
  }, [isOpen, trainingGoal, currentUser.id]);

  if (!isOpen) return null;

  const persistGoal = (goal: TrainingGoal) => {
    try {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem(getGoalCacheKey(currentUser.id), goal);
      }
    } catch {
  ...
  const handleForceRecalc = () => {
    try {
      if (typeof window !== 'undefined') {
        window.localStorage.removeItem(getReportCacheKey(currentUser.id));
      }
    } catch {
      // Si falla, seguimos con recalculo igual
    }
    setData(null);
    setError(null);
    setFormattedError(null);
    isLoadingRef.current = false;
    setForceRecalcTick((tick) => tick + 1);
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-[#0a0a0a] border border-white/10 rounded-[2rem] shadow-2xl flex flex-col h-[92vh] animate-in zoom-in-95 duration-300 text-white overflow-hidden ring-1 ring-white/5">
          ...
          <div className="flex items-center gap-2">
            {trainingGoal && (
              <button
                ...
                className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full transition-colors"
              >
                Objetivo: {getGoalLabel(trainingGoal)}
              </button>
            )}
            <button
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#050505] p-6">
            {!trainingGoal || isSelectingGoal ? (
                <div className="flex flex-col items-center justify-center h-full text-center space-y-6">
                    <div className="space-y-2">
                        <p className="text-[10px] font-mono text-primary uppercase tracking-widest">Selecciona objetivo</p>
                        <h4 className="text-lg font-black text-white uppercase tracking-wide">Define tu enfoque</h4>
                        <p className="text-xs text-zinc-500">El informe y el plan se ajustan a tu objetivo principal.</p>
                    </div>
                    <div className="grid grid-cols-1 gap-3 w-full max-w-sm">
                        {TRAINING_GOALS.map((goal) => (
                            <button
                              ...
                              className="text-left p-4 rounded-2xl border border-white/10 bg-zinc-900/40 hover:bg-zinc-900/60 hover:border-white/20 transition-colors"
                            >
                                <div className="text-sm font-black uppercase tracking-widest text-white">{goal.label}</div>
