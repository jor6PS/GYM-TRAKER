    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full max-w-lg bg-surface border-t sm:border border-white/10 rounded-t-3xl sm:rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col h-[90vh] sm:max-h-[85vh] animate-in slide-in-from-bottom-10 duration-300">
        ...
        <div className="flex-1 overflow-y-auto bg-zinc-900 p-4 custom-scrollbar">
                        ...
                        <>
                            ...
                            <div className="flex-1 overflow-y-auto space-y-1 custom-scrollbar pr-1">
                                {filteredLibrary.map((ex, i) => {
                                    ...
                                    <button
                                        key={i}
                                        onClick={() => handleSelectExercise(ex.es)}
                        ...
                        {plans.length === 0 ? (
                            ...
                            plans.map((plan) => (
                                <button
                                    ...
                                    className="w-full bg-black border border-white/10 hover:border-yellow-400/30 p-4 rounded-xl text-left transition-all group flex justify-between items-center"
                                >
                                    <div>
                                        <div className="text-white font-bold text-sm group-hover:text-yellow-400 transition-colors">{plan.name}</div>
                                        <div className="text-xs text-zinc-500 mt-1">{plan.exercises.length} Exercises</div>
                                    </div>
                                    <Plus className="w-5 h-5 text-zinc-600 group-hover:text-yellow-400" />
                                </button>
                            ))
                        )}
                    </div>
            ...
            {/* --- TAB: OVERVIEW (REVIEW & SAVE) --- */}
            {activeTab === 'overview' && (
                <div className="flex flex-col h-full">
                    {sessionExercises.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-zinc-500">
                            <Layers className="w-12 h-12 mb-3 opacity-20" />
                            <p className="text-sm">{t('session_empty')}</p>
                            <button onClick={() => setActiveTab('library')} className="mt-4 text-xs font-bold text-green-400 hover:underline">
                                {t('go_to_lib')}
                            </button>
                        </div>
                    ) : (
                        <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar">
                                ...
                                <div key={i} className="bg-black border border-white/10 rounded-xl p-3 relative group hover:border-primary/30 transition-colors">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <div className="flex gap-2">
                                            <button onClick={() => setEditingItem({ index: i, data: ex })} className="text-zinc-600 hover:text-white p-1">
                                                <Pencil className="w-3.5 h-3.5" />
                                            </button>
                                            <button onClick={() => removeSessionExercise(i)} className="text-zinc-600 hover:text-red-500 p-1">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {ex.sets.map((s, idx) => (
                                            <div key={idx} className="bg-zinc-900 border border-white/5 rounded px-2 py-1 text-[10px] text-zinc-400 font-mono">
                                                {s.distance ? (
                                                    <span>{s.distance}km {s.time}</span>
                                                ) : (
                                                    <span><span className="text-white font-bold">{s.weight}</span>kg x {s.reps}</span>
                                                )}
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <button 
                                onClick={() => setActiveTab('library')}
                                className="w-full py-3 border border-dashed border-white/10 rounded-xl text-xs text-zinc-500 hover:text-white transition-colors uppercase font-bold flex items-center justify-center gap-2"
                            >
                    ...
                    {sessionExercises.length > 0 && (
                        <div className="pt-4 mt-auto border-t border-white/10 shrink-0">
                            <button 
                                ...
                                className="w-full bg-primary text-black font-bold py-4 rounded-xl shadow-glow active:scale-95 transition-transform uppercase text-sm flex items-center justify-center gap-2"
                            >
                                <Save className="w-5 h-5" /> {t('save_session')}
                            </button>
                        </div>
                    )}
                </div>
                        ...
                        >
                            <div className="flex justify-between items-center mb-2">
                                <div className="text-xs font-bold text-blue-400 uppercase tracking-wider flex items-center gap-2">
                                    <Clock className="w-3 h-3" /> {format(new Date(h.date), 'MMM do, yyyy')}
                                </div>
