        ...
        <div className="p-4 space-y-3 max-h-[60vh] overflow-y-auto bg-background custom-scrollbar">
          ...
          {sets.map((set, idx) => (
            <div key={idx} className="grid grid-cols-12 gap-2 items-center">
               ...
               {exerciseType === 'cardio' && (
                   <>
                    <div className="col-span-4"><input type="number" value={set.distance || ''} onChange={(e) => handleSetChange(idx, 'distance', Number(e.target.value))} className="w-full bg-surface border border-border rounded p-2 text-center text-sm font-bold text-blue-400 focus:border-blue-400 focus:outline-none" placeholder="km" /></div>
                    <div className="col-span-5"><input type="text" value={set.time || ''} onChange={(e) => handleSetChange(idx, 'time', e.target.value)} className="w-full bg-surface border border-border rounded p-2 text-center text-sm text-text focus:border-blue-400 focus:outline-none" placeholder="MM:SS" /></div>
                   </>
               )}
